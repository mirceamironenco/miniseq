...
class TyroFlatSubcommandHelpFormatter(_argparse_formatter.TyroArgparseHelpFormatter):
    ...
    def add_usage(self, usage, actions, groups, prefix=None) -> None:
        # NB: This changes the `usage: ` line at the top of the CLI output when
        # --choices is passed in.
        aggregated_subcommand_group = []
        for action_name, sub_parser in self.collect_subcommands_parsers(
            actions
        ).items():
    ...
    @override
    def add_arguments(self, action_group) -> None:
        # Note: displays all options for a command.
        ...
        for action in action_group:
            ...
            for action_name, sub_parser in subcommands_parsers.items():
                ...
                for sub_action_group in sub_parser._action_groups:
                    ...
                    if len(sub_group_actions) > 0:
                        if any(
                            ...
                            # NB: ^ commenting this out shows only choices and
                            # not arguments of choices.
                            continue
    ...
    def _format_usage(
        ...
        # TODO: Modified the OPTIONS string to not show [SUBCOMMAND:CHOICE OPTIONS]
        # but just [OPTIONS]. Maybe make this configurable.
        # TODO: There should be some text width limit here for the --choices
        # command since there might be subcommands with a lot of options, and we are
        # already displaying all the options in the group helptexts.
        # TODO: Also when --choices is passed in the [OPTIONS] usage text
        # appears before the subcommands, although we are actually forcing
        # Consolidate, so this is not correct.
        if len(actions) > 4:
            new_actions = []
